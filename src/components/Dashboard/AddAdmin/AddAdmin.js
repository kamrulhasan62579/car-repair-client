...
export default function AddAdmin() {
 ...
 if (img) {
  ...
  const onSubmit = data => {
    ...
    console.log(newData);
    fetch("http://localhost:3011/addAdmin", {
      method: 'POST',
  ...
  return (
   <div className="d-flex justify-content-center">
     ...
     <label htmlFor="">Admin Photo</label>
      ...
      <input className="form-control" placeholder="Admin Name" {...register("adminName", { required: true })} />
      {errors.adminName && <span className="error">Admin Name is required</span>}
      <br/>

      <label htmlFor="">Admin Email Account</label>
      <input className="form-control" placeholder="Email Account" {...register("email", { required: true })} />
      {errors.email && <span className="error">Email is required</span>}
      <br/>

      <label htmlFor="">Admin Country</label>
       <input className="form-control" placeholder="Country" {...register("country", { required: true })} />
